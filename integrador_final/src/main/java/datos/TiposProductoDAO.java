...
public class TiposProductoDAO {
    ...
    private static final String SQL_READ_BY_TIPO_ALL = "SELECT * FROM tipos_producto WHERE tip_id = ?";
    ...
    public List<TiposProducto> seleccionarTodos() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                boolean borrar = borrarTipo(tip_id);
                
                tiposProducto = new TiposProducto(tip_id, tip_descripcion, borrar);
    ...
    public int borrar(int idBorrar) {
        ...
        try {
            conn = getConexion();
            registros = temp.contarPorTipo(idBorrar);
            if (registros == 0) {
    ...
    public boolean borrarTipo(int tipoId) {
        boolean borrar = false;
        int total = 1;
        total = new ProductosDAO().contarPorTipo(tipoId);
        if (total == 0) {
            borrar = true;
        }
        return borrar;
    ...
    public TiposProducto seleccionarPorId(int idBuscado) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        TiposProducto tempTipo = null;
        try {
            conn = getConexion();
            stmt = conn.prepareStatement(SQL_READ_BY_TIPO_ALL);
            stmt.setInt(1, idBuscado);
            rs = stmt.executeQuery();
            while (rs.next()) {
                int tip_id = rs.getInt(1);
                String tip_descripcion = rs.getString(2);
                tempTipo = new TiposProducto(tip_id, tip_descripcion);
            }
        } catch (SQLException ex) {
            ex.printStackTrace(System.out);
        } finally {
